...
class BinarySearchTree:
    ...
    def dibujarArbol(self):
        lines = []
        self._dibujarArbolRecursivamente(self.root, 0, lines)
        return "\n".join(lines)

    def _dibujarArbolRecursivamente(self, node, level, lines):
        if node is not None:
            self._dibujarArbolRecursivamente(node.right, level + 1, lines)
            lines.append("\t" * level + str(node.key))
            self._dibujarArbolRecursivamente(node.left, level + 1, lines)
...
for i in range(1, casos + 1):
    ...
    results.append(bst.dibujarArbol())
